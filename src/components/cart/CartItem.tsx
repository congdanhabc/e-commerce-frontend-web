...
interface CartItemProps {
  id: string; // ID của dòng sản phẩm (lineId)
  name: string;
  price: number;
  quantity: number;
  image: string;
...
export function CartItem({
  ...
  return (
    <div className="flex gap-4">
      {/* Product Image */}
      <Link to={`/products/${link}`} className="w-[124px] h-[124px] flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
        <img src={image} alt={name} className="w-full h-full object-cover" />
      </Link>

      {/* Product Details */}
      <div className="flex-1 flex justify-between">
        <Link to={`/products/${link}`} className="flex flex-col justify-between">
          <div>
            <h3 className="text-xl font-bold mb-1">{name}</h3>
          </div>
          <p className="text-2xl font-bold">{formatPriceVND(price)}</p>
        </Link>
